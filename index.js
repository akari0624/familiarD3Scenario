...
(function () {
  ...
  var getClientRectWidthAndHeight = function getClientRectWidthAndHeight(svgDom) {
    var box = svgDom.getBoundingClientRect();
    return {
      width: box.width,
      height: box.height
    };
  ...
  var flatMap = function flatMap(arr, accessor) {
    return arr.reduce(function (acc, curr) {
      var level2Arr = accessor(curr);
      return acc.concat(level2Arr);
    }, []);
  };

  D3Select.prototype.transition = transition;

  function drawBottomXAxis(instance) {
    var svgHeight = instance.svgHeight,
        d3ishSVG = instance.d3ishSVG,
        xScaleBandG = instance.xScaleBandG,
        margin = instance.margin,
        isFirstDraw = instance.isFirstDraw;

    if (isFirstDraw) {
      d3ishSVG.append('g').attr('class', 'x_axis').attr('transform', "translate(" + margin.left + ", " + (svgHeight - margin.bottom) + ")").call(axisBottom(xScaleBandG));
    } else {
      d3ishSVG.select('.x_axis').attr('transform', "translate(" + margin.left + ", " + (svgHeight - margin.bottom) + ")").call(axisBottom(xScaleBandG));
    }
  }

  function drawLeftYAxis(instance) {
    var yScaleLinear = instance.yScaleLinear,
        d3ishSVG = instance.d3ishSVG,
        isFirstDraw = instance.isFirstDraw,
        margin = instance.margin;
  ...
  function () {
    ...
    function BarCharts(svgDom) {
      ...
      this._prepareAxisAndScale = function () {
        var _a = _this,
            ...
            height = _a.svgHeight,
            data = _a.data,
            margin = _a.margin; //  給g用的

        _this.xScaleBandG = band() // range 參數array的第二個數  是要給入  所有g最後會用到多少空間，而不是一個g會用到多少空間
        // d3會自己這次進來的資料應該有多少個g, 然後d3會去除出各個g應該分配多少寬度
        .range([0, width - margin.left - margin.right]).padding(0.1).domain(data.map(function (d) {
          return d.date;
        })); // 給rect用的  一個g裡可能包多個rect i.e. grouped bar chart

        var oneData = data.slice(0, 1);
        var forXScaleDomain = flatMap(oneData, function (d) {
          return d.categories.map(function (c) {
            return c.name;
          });
        });
        _this.xScaleBandRect = band() // 同理 這裡range 參數array的第二個數  是要給入  所有rect最後會用到多少空間，而不是一個rect會用到多少空間
        // d3會自己這次進來的資料應該有多少個rect, 然後d3會去除出各個g應該分配多少寬度
        // 所以這個array裡的第二個參數等於是這一次一塊g的寬度
        .range([0, _this.xScaleBandG.bandwidth()]) // 給入這次有的category name
        .domain(forXScaleDomain);
        _this.yScaleLinear = linear$1().range([height - margin.bottom, margin.top]).domain([0, max(flatMap(data, function (d) {
          return d.categories.map(function (c) {
            return c.value;
          });
        }))]);
      ...
      this.drawNewEnterRectInTheGs = function () {
        _this.theNewEnterBarsRects = _this.newEnterGsThatWrapTheRects.selectAll('rect').data(function (d) {
          return d.categories;
        }).enter().append('rect').attr('width', _this.xScaleBandG.bandwidth() / _this.categoryLength).attr('x', function (d) {
          return _this.xScaleBandRect(d.name);
        }).attr('y', _this.svgHeight - _this.margin.bottom).style('fill', function (d) {
          return _this.tColors(d.name);
        }).on('click', _this.onRectClick);
      };

      this.setRectTransition = function () {
        _this.theNewEnterBarsRects.transition().duration(1000) //由下往上長
        .attr('y', function (d) {
          return _this.yScaleLinear(d.value);
        }).attr('height', function (d) {
          return _this.svgHeight - _this.margin.bottom - _this.yScaleLinear(d.value);
        });
      };

      this.update_updateExistedBar = function () {
        // 移動g
        _this.dataBinds.attr('transform', function (d) {
          var result = _this.xScaleBandG(d.date);

          return "translate( " + (result + _this.margin.left) + ", 0 )";
        }); // update not enter rect


        _this.dataBinds.selectAll('rect').data(function (d) {
          return d.categories;
        }).style('fill', function (d) {
          return _this.tColors(d.name);
        }).transition().duration(1000).attr('width', _this.xScaleBandG.bandwidth() / _this.categoryLength).attr('x', function (d) {
          return _this.xScaleBandRect(d.name);
        }) //由下往上長
        .attr('y', function (d) {
          return _this.yScaleLinear(d.value);
      ...
      this.exit_removeNoDataCorrespondedBar = function () {
        //  再給資料然後 再比對rect一次  刪掉沒有資料對應的rect
        var existingRect = _this.dataBinds.selectAll('rect').data(function (d) {
          return d.categories;
        }).exit();
      ...
      this.draw = function (data) {
        _this.data = data;
        _this.categoryLength = data[0].categories.length;

        _this._prepareAxisAndScale();

        drawBottomXAxis(_this);
        drawLeftYAxis(_this);

        if (_this.isFirstDraw) {
          _this.isFirstDraw = false;
        }

        _this.bindDataToG();

        _this.exit_removeNoDataCorrespondedBar();

        _this.enter_drawNewGsThatToWrapTheRect();

        _this.drawNewEnterRectInTheGs();

        _this.setRectTransition();
    ...
    BarCharts.prototype.initD3ishSVG = function () {
      if (!this.d3ishSVG) {
        this.d3ishSVG = D3Select(this.svgDom);
      }

      this.d3ishSVG.attr('width', this.svgWidth).attr('height', this.svgHeight).attr('transform', "translate(" + this.margin.left + ", " + this.margin.top + ")");
    };

    BarCharts.prototype.getTheD3ishSVG = function () {
    ...
    */
    ...
    BarCharts.prototype.enter_drawNewGsThatToWrapTheRect = function () {
      var _this = this; // 包住 rect的 g


      this.newEnterGsThatWrapTheRects = this.dataBinds.enter().append('g').attr('class', wrapRect_G_ClassNameWithoutDot).attr('transform', function (d) {
        var result = _this.xScaleBandG(d.date);
  ...
  function drawRightYAxis(instance) {
    var svgWidth = instance.svgWidth,
        d3ishSVG = instance.d3ishSVG,
        yMaxScaleLinear = instance.yMaxScaleLinear,
        isFirstDraw = instance.isFirstDraw,
        margin = instance.margin;
  ...
  function () {
    function LineCharts(svgDom) {
      ...
      this.doDataBind = function () {
        var _a = _this,
            d3ishSVG = _a.d3ishSVG,
            data = _a.data;
        _this.dataBinds = d3ishSVG.selectAll('.pathWrapperG').data(data);
      };

      this.drawTheLine = function (lineColors) {
        var tLineFunc = lineFunc(_this);

        _this.dataBinds.enter().append('g').attr('class', 'pathWrapperG').each(function (d) {
          D3Select(this) // 這個this就是 各個包裹著各條line的g
          .append('path').attr('d', function (b) {
            return tLineFunc(d.datas);
          }).attr('stroke', lineColors(d.name)).style('fill', 'none').call(transition$1);
        });
      };

      this.update_line = function (lineColors) {
        var tLineFunc = lineFunc(_this);

        _this.dataBinds.select('path').attr('d', function (b) {
          return tLineFunc(b.datas);
        }).attr('stroke', function (d) {
          return lineColors(d.name);
        }).style('fill', 'none').call(transition$1);
      };
    ...
    LineCharts.prototype.initD3ishSVG = function () {
      if (!this.d3ishSVG) {
        this.d3ishSVG = D3Select(this.svgDom);
        this.d3ishSVG.attr('width', this.svgWidth).attr('height', this.svgHeight).attr('transform', "translate(" + this.margin.left + ", " + this.margin.top + ")");
      }
    ...
    */
    ...
    LineCharts.prototype.prepareLineralAndAxis = function () {
      ...
      this.xScaleBand = band().range([0, width - margin.left - margin.right]).padding(0.1).domain(flatMap(data, function (d) {
        return d.datas.map(function (dd) {
          return dd.date;
        });
      }));
      this.yMaxScaleLinear = linear$1().range([height - margin.bottom, margin.top]).domain([0, max(flatMap(data, function (d) {
        return d.datas.map(function (dd) {
          return dd.value;
        });
      }), function (d) {
  ...
  function () {
    ...
    PieCharts.prototype.initD3ishSVG = function () {
      if (!this.d3ishSVG) {
        this.d3ishSVG = D3Select(this.svgDom);
      }
    };

    PieCharts.prototype.setD3ishSVGFromOtherChart = function (d3ishSvgFromOtherChart) {
      this.d3ishSVG = d3ishSvgFromOtherChart;
    };

    PieCharts.prototype.initNeededFunc = function () {
      this.arcFunc = arc().innerRadius(this.pieInnerRadius).outerRadius(this.pieRadius - 40);
      this.pieLayout = pie().sort(null).value(function (d) {
        return d.value;
      });
    ...
    PieCharts.prototype.appendOutWrapperGAndSetTheStartDrawCenter = function () {
      if (this.isFirstBind) {
        this.theWholeWrapperG = this.d3ishSVG.append('g').attr('transform', "translate(" + this.svgWidth / 2 + ", " + this.svgHeight / 2 + ")");
        this.isFirstBind = false;
      }
    };

    PieCharts.prototype.startToBindBeforeEnter = function () {
      this.dataBinds = this.theWholeWrapperG.selectAll('.arc').data(this.pieLayout(this.data));
    };
    ...
    PieCharts.prototype.doExitRemove = function () {
      this.dataBinds.exit().remove();
    };

    PieCharts.prototype.draw = function (data) {
      this.data = data;
      this.initD3ishSVG();
      this.initNeededFunc();
  ...
  var transToLineData = function transToLineData(tdata) {
    var landLine = tdata.reduce(function (acc, curr) {
      var d = {
        date: '',
        value: 0
      };
    ...
    var wholeBuildLine = tdata.reduce(function (acc, curr) {
      var d = {
        date: '',
        value: 0
      };
    ...
    var buildingLine = tdata.reduce(function (acc, curr) {
      var d = {
        date: '',
        value: 0
      };
  ...
  setTimeout(function () {
    ...
    var linesData3 = transToLineData(barTestData3);
